...
public class DragRaceGame {
	...
	public static void main(String[] args) {
		...
		int y = (screenSize.height - frame.getHeight()) / 2 - 5; // Calculate y coordinate to center frame vertically
																	// (slightly adjusted)
...
class GameHolder extends JPanel {
	...
	public GameHolder() {
		...
		HighScorePanelAfter highScoresPanelAfter = new HighScorePanelAfter(this, layout); // Create post-game high
																							// scores panel
...
// It includes animation transitions, intro sounds, skip/mute buttons, and
...
class WelcomePagePanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (!gifOrNo) {
			...
			} else if (rightButtonPressed && y > 855 && y < 896 && x > 587 && x < 964) // Right button released inside
																						// area
			{
...
class InstructionPanel extends JPanel {
	...
	private final Timer textTimer = new Timer(40, e -> {
		...
		if (hue > 1f)
			hue = 0f; // Reset hue when it exceeds 1 (HSB wraparound)
		repaint(); // Trigger repaint to update gradient text color
	});

	// Constructor initializes panel with parent references and starts animation
	// timer
			...
			public void actionPerformed(ActionEvent e) {
				if (!agreementChecked) {
					...
					javax.swing.JOptionPane.showMessageDialog(parent,
							"Please agree to the terms and conditions to proceed.", "Agreement Required",
...
class CarChoosePanel extends JPanel implements MouseListener, MouseMotionListener {
	// This class represents the car selection panel where players can choose their
	// car and enter their name.
	...
	public void addComponents() {
		try {
			...
			carOptionsNoBackgroundOriginal = ImageIO.read(new File("CarOptionsClearBackground.png")); // Load car
																										// options image
																										// without
																										// background
		} catch (IOException e) {
	...
	public void paintComponent(Graphics g) {
		...
		if (xClick != 0) {
			...
			g.drawImage(carOptionsNoBackgroundOriginal, 662, 131, 662 + carWidth, 131 + carHeight, xClick, yClick,
					xClick + carWidth, yClick + carHeight, this); // Draw the cropped image
		} else {
...
class GamePanel extends JPanel {
	...
	GamePanel(JPanel gameHolder, CardLayout layout) {
		...
		// Initialize sound players for correct, incorrect answers and race background
		// music
		...
		// Start button action logic: start timer and initialize questions or restart
		// game
		...
		questionOneButton.addActionListener(
				e -> handleAnswer(0, questionOneButton, questionTwoButton, questionThreeButton, questionFourButton));
		questionTwoButton.addActionListener(
				e -> handleAnswer(1, questionOneButton, questionTwoButton, questionThreeButton, questionFourButton));
		questionThreeButton.addActionListener(
				e -> handleAnswer(2, questionOneButton, questionTwoButton, questionThreeButton, questionFourButton));
		questionFourButton.addActionListener(
				e -> handleAnswer(3, questionOneButton, questionTwoButton, questionThreeButton, questionFourButton));
	...
	// Update the progress bar to current car1 logical position capped at finish
	// line
	...
	// Load car images and set opponent speed based on opponent car type and
	// difficulty level
	public void getCar() {
	...
	// Start the main game timer that updates car positions, track scrolling, and
	...
	public void startTimer() {
		...
		gameTimer = new Timer(16, e -> {
			...
			if (car1LogicalPos >= FINISH_LINE && car2LogicalPos < FINISH_LINE) {
				...
				JOptionPane.showMessageDialog(GamePanel.this, "You Win!", "Race Result",
			...
			// Check if opponent wins or both finish simultaneously
				...
				JOptionPane.showMessageDialog(GamePanel.this, "You Lose!", "Race Result",
						JOptionPane.INFORMATION_MESSAGE, lose);
				next.setVisible(true);
				start.setVisible(false);
			}
	...
	// Paint the game panel, including track and cars, with random shaking effect
	...
	// Handle player's answer selection, apply speed boost or slowdown, update
	// questions, and manage UI feedback
...
class TugOfWarPanel extends JPanel {
	...
	private void getImages() {
		try {
			...
			JOptionPane.showMessageDialog(this, "Error loading game images: " + e.getMessage(), "Image Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}

	private void runCountdown(int seconds, Runnable onComplete) {
		JDialog dialog = new JDialog(SwingUtilities.getWindowAncestor(this), "Get Ready",
...
class SoundPlayer {
	...
	// Play the sound
	public void play() {
		if (clip != null && !isMuted) { // Check if not muted
	...
	public void setVolume(float volume) {
		if (volumeControl != null && !isMuted) { // Check if not muted
...
// Panel shown at the end of the game, with animation and buttons for replay or
// quitting
